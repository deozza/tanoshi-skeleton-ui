...
describe('paragraph customization', () => {
	...
	const testCases_globalTheme_parameters = ['black', 'white', 'primary', 'secondary', 'success', 'danger', 'warning', 'info'];
	const testCases_globalTheme_expectedRgbValues = ['rgb(0, 0, 0)', 'rgb(255, 255, 255)', 'rgb(0, 123, 255)', 'rgb(108, 117, 125)', 'rgb(40, 167, 69)', 'rgb(220, 53, 69)', 'rgb(255, 193, 7)', 'rgb(23, 162, 184)'];
	...
	const testCases_customTheme_parameters = ['paragraph-text-black', 'paragraph-text-white', 'paragraph-text-primary', 'paragraph-text-secondary', 'paragraph-text-success', 'paragraph-text-danger', 'paragraph-text-warning', 'paragraph-text-info'];
	const testCases_customTheme_expectedRgbValues = ['rgb(25, 25, 25)', 'rgb(250, 250, 250)', 'rgb(10, 123, 255)', 'rgb(118, 117, 125)', 'rgb(50, 167, 69)', 'rgb(230, 53, 69)', 'rgb(155, 193, 7)', 'rgb(33, 162, 184)'];
	...
	const testCases_fontWeight_parameters = ['extralight', 'light', 'regular', 'medium', 'bold', 'extrabold'];
	...
	testCases_fontWeight_parameters.forEach((fontWeight, index) => {
		test(`paragraph with font weight ${fontWeight}`, async ({ mount }) => {
			...
			await expect(component).toHaveCSS('font-weight', testCases_fontWeight_expectedFontWeightValues[index] );
		});
	});
	...
	testCases_textAlignment_parameters.forEach((textAlign, index) => {
		test(`paragraph with text alignment ${textAlign}`, async ({ mount }) => {
			...
			await expect(component).toHaveCSS('text-align', testCases_textAlignment_expectedTextAlignValues[index] );
		});
	});

	const testCases_textTransform_parameters = ['uppercase', 'lowercase', 'capitalize'];
	const testCases_textTransform_expectedTextTransformValues = ['uppercase', 'lowercase', 'capitalize'];
	...
	testCases_textTransform_parameters.forEach((textTransform, index) => {
		test(`paragraph with text transform ${textTransform}`, async ({ mount }) => {
			...
			await expect(component).toHaveCSS('text-transform', testCases_textTransform_expectedTextTransformValues[index] );
		});
	});

	const testCases_textDecoration_parameters = ['underline', 'line-through', 'no-underline'];
	const testCases_textDecoration_expectedTextDecorationValues = ['underline', 'line-through', 'no-underline'];
	...
	testCases_textDecoration_parameters.forEach((textDecoration, index) => {
		test(`paragraph with text decoration ${textDecoration}`, async ({ mount }) => {
			...
			await expect(component).toHaveCSS('text-decoration', testCases_textDecoration_expectedTextDecorationValues[index] );
		});
	});
